...
GROQ_MODEL    = os.environ.get("GROQ_FAST_MODEL", "llama-3.1-8b-instant")
...
async def _llm(prompt: str, max_tokens: int = 400) -> str:
    """Gemini 2.5 Flash → Groq → Ollama fallback chain."""
    if GEMINI_API_KEY:
        try:
            async with httpx.AsyncClient(timeout=20) as c:
                r = await c.post(
                    ...
                    json={"model": GEMINI_MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": max_tokens},
                )
...
async def _gemini_opener(lead: dict) -> str:
    """Generate a hyper-personalized first sentence for step-1 emails using Gemini."""
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as c:
            r = await c.post(
                GEMINI_URL,
                headers={"Authorization": f"Bearer {GEMINI_API_KEY}"},
                json={"model": GEMINI_MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 60},
            )
        if r.status_code == 200:
            opener = r.json()["choices"][0]["message"]["content"].strip().strip('"').strip("'")
            if opener and len(opener) < 200:
                return opener
    except Exception as e:
...
# Niche-specific subject lines for step 1 — more personal than generic "Quick idea"
NICHE_SUBJECTS_STEP1 = {
    "HVAC":              "AC calls after 5pm — where do they go?",
    "Plumbing":          "2am plumbing emergency — who answers for you?",
    "Electrical":        "After-hours electrical inquiries — captured or lost?",
    "Roofing":           "Storm damage calls you might be missing",
    "Dental":            "After-hours patient questions for {name}",
    "Veterinary":        "Pet emergencies after hours — are you capturing those calls?",
    "Vet ":              "Pet emergencies after hours — are you capturing those calls?",
    "Auto Repair":       "Estimate requests coming in after you close",
    "Auto repair":       "Estimate requests coming in after you close",
    "Landscaping":       "Spring booking season — are you capturing every lead?",
    "Pest Control":      "Pest calls at night — captured or lost?",
    "Pest control":      "Pest calls at night — captured or lost?",
    "Law Firm":          "New client inquiries after 5pm",
    "Law firm":          "New client inquiries after 5pm",
    "Insurance":         "Quote requests outside business hours",
    "Cleaning":          "New cleaning client requests — automated follow-up",
    "Salon":             "Appointment requests while you're with clients",
    "Gym":               "Membership inquiries after hours",
    "Real Estate":       "Property inquiries at midnight",
    "Property Management": "Tenant maintenance requests at 11pm — who answers?",
    "Property management": "Tenant maintenance requests at 11pm — who answers?",
}
...
DEMO_URL = "https://ringcatch.io/book"
...
STEP_INSTRUCTIONS = {
    1: (
        ...
        "\"See how it would work specifically for {business_name}: " + DEMO_URL + "\"\n"
    ...
    2: (
        ...
        "Second sentence MUST end with: \"60-second demo tailored to {business_name}: " + DEMO_URL + "\"\n"
...
async def chat_start(payload: dict):
    ...
    try:
        opener = await _llm(prompt)
    except Exception as exc:
        logger.warning(f"chat/start failed: {exc}")
        opener = (
...
async def chat_message(payload: dict, background_tasks: BackgroundTasks):
    ...
    try:
        reply = await _llm(prompt)
    except Exception as exc:
        logger.warning(f"chat/message phase={phase} failed: {exc}")
        reply = "Sorry, hit a snag — give me just a second and try again."
...
async def demo_followup(payload: dict, background_tasks: BackgroundTasks):
    ...
    body = (
        ...
        f"If you want to move forward (or just have questions), here's your demo link to share: {DEMO_URL}\n\n"
...
async def _generate_email(lead: dict, step: int) -> str:
    ...
    opener_hint = ""
    if step == 1:
        opener = await _gemini_opener(lead)
        if opener:
            opener_hint = (
    ...
    FALLBACK_BODIES = {
        1: (
            ...
            f"See exactly how it would work for {lead.get('business_name', 'your business')}: {DEMO_URL}\n\n"
            f"Alex\nRingCatch"
        ),
        2: (
            ...
            f"60-second demo tailored to {lead.get('business_name', 'you')}: {DEMO_URL}\n\nAlex"
        ),
        3: (
            f"Last note from me on this — if the timing's ever right: {DEMO_URL}\n\nAlex"
        ),
    }
    try:
...
def _build_email_html(body: str, video_url: str | None = None, niche: str = "") -> str:
    ...
    html_body = "".join(
        ...
        for p in paragraphs
        if DEMO_URL not in p and "Alex" not in p[:10]
    )
    return (
        ...
        f"<a href='{DEMO_URL}' style='display:inline-block;background:#22d3ee;color:#0b0b14;"
...
async def _dispatch_email(lead: dict, body: str, step: int) -> bool:
    ...
    if raw_subject == "__NICHE__":
        niche_key = lead.get("niche", "")
        raw_subject = next(
            (v for k, v in NICHE_SUBJECTS_STEP1.items() if k.lower() in niche_key.lower()),
            "A quick idea for {name}",
        )
    subject = raw_subject.format(name=lead.get("business_name", "your business"))
