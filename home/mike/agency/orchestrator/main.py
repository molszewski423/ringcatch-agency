...
async def tool_diagnose_pipeline() -> str:
    ...
    for a in agents:
        if a["status"] != "online":
            ...
            suggestions.append(f"→ Run get_agent_logs('{a['name']}') to diagnose")
        ...
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                resp = await client.post(
                    ...
                    json={"model": model, "messages": _to_groq_messages(_trim_for_groq(messages)), "tools": TOOLS, "tool_choice": "auto"},
                )
            if resp.status_code == 200:
                msg = resp.json()["choices"][0]["message"]
                logger.info(f"LLM: {label} (attempt {attempt+1})")
...
async def call_llm(messages: list) -> dict:
    """Try Gemini → Cerebras → Ollama → Groq (emergency). Returns first successful response."""
    errors = []

    # 1. Gemini 2.5 Flash (primary — Google AI Pro)
    if GEMINI_API_KEY:
        result = await _try_openai_provider(GEMINI_URL, GEMINI_API_KEY, GEMINI_MODEL, messages, f"gemini/{GEMINI_MODEL}")
        if result is not None:
            return result
        errors.append("Gemini: rate-limited/unavailable")

    # 2. Cerebras llama3.1-8b (paid fallback)
    if CEREBRAS_API_KEY:
        result = await _try_openai_provider(CEREBRAS_URL, CEREBRAS_API_KEY, CEREBRAS_MODEL, messages, f"cerebras/{CEREBRAS_MODEL}")
        if result is not None:
            return result
        errors.append("Cerebras: rate-limited/unavailable")

    # 3. Ollama qwen2.5:14b (local, free)
    try:
...
async def chat(session_id: str, user_message: str) -> str:
    ...
    for round_num in range(MAX_TOOL_ROUNDS):
        try:
            ...
            fallback = (
                "I'm temporarily unavailable — both Groq and Ollama are unreachable right now. "
...
def _webchat_sanitize(text: str) -> str:
    """Strip leaked tool call JSON and any stray JSON objects from webchat replies."""
    import re
    # Remove <function=name>...</function> markup (some models leak this format)
    text = re.sub(r'<function=\w+>.*?</function>', '', text, flags=re.DOTALL)
    # Remove [function_call] style markers
    text = re.sub(r'\[function[_\s]call[^\]]*\]', '', text, flags=re.IGNORECASE)
    # Remove any {...} blocks that look like tool arguments
...
async def _call_webchat_llm(messages: list) -> dict:
    """Try Gemini → Cerebras → Ollama → Groq for webchat."""
    trimmed = _to_groq_messages(_trim_for_groq(messages, max_tokens=6000))
    for label, url, key, model in [
        (f"gemini/{GEMINI_MODEL}",      GEMINI_URL,    GEMINI_API_KEY,    GEMINI_MODEL),
        (f"cerebras/{CEREBRAS_MODEL}",  CEREBRAS_URL,  CEREBRAS_API_KEY,  CEREBRAS_MODEL),
        (f"groq/{GROQ_MODEL}",          GROQ_URL,      GROQ_API_KEY,      GROQ_MODEL),
    ]:
        if not key:
            continue
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                resp = await client.post(url, headers={"Authorization": f"Bearer {key}"},
                                         json={"model": model, "messages": trimmed, "tools": WEBCHAT_TOOLS, "tool_choice": "auto"})
            if resp.status_code == 200:
                logger.info(f"Webchat LLM: {label}")
                return _normalize_openai_response(resp.json()["choices"][0]["message"], label)
            logger.warning(f"Webchat {label} HTTP {resp.status_code} — trying next")
        except Exception as e:
            logger.warning(f"Webchat {label} error: {e} — trying next")
    ...
    if resp.status_code != 200:
        raise Exception(f"All webchat LLM providers failed")
...
async def webchat(session_id: str, user_message: str) -> str:
    ...
    for _ in range(6):
        try:
            ...
            logger.warning(f"Webchat LLM error: {e}")
            return "Sorry, I'm having a moment. Try again or email us at hello@ringcatch.io!"
    ...
    return "I'd love to keep chatting — why not book a quick 15-min call? https://ringcatch.io/book"
